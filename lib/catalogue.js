...
//////PUBLIC METHODS//
	...
	/**
	 * Reads a attribute from a node in the catalogue.
	 * @param connection string - Id of the connection to get the node for.
	 * @param nodePath string - Path to the node which has the attribute.
	 * @param attribute string - Name of the attribute to return the value of.
	 */
	this.ReadNodeAttribute = function( connection, nodePath, attribute ){
		//TODO will the hold up when a injection attack is used?
		var node = _connections.get( "connection[@id='" + connection + "']/" + nodePath );
		if( node ){
